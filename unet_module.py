...
class Unet_Main:
    def __init__(self):
        ...
        self.net.load_state_dict(torch.load('water_depth_unet_500.pth', map_location=self.device))
        self.net.eval()

    def preprocess(self, img_nd, scale, img_size=(300,300), n_classes=1):
    ...
    def run(self, full_img,
        ...
        scores_resized = cv2.resize(score2, (200,860))

        # Bounding box.
        vertical_indicies = np.where(np.any(scores_resized, axis=1))[0]
        if vertical_indicies != []:
            water_line = vertical_indicies[0]


        if vertical_indicies != []:
            y = vertical_indicies[-1]
